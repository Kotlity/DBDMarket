...
class MainCategoryRepositoryImplementation @Inject constructor(private val firebaseFirestore: FirebaseFirestore): MainCategoryRepository {
    ...
    override fun getBeneficialProductsFromFirebaseFirestore(onSuccess: (QuerySnapshot) -> Unit, onFailure: (Exception) -> Unit) {
        firebaseFirestore.collection(FIREBASE_FIRESTORE_PRODUCTS_COLLECTION).whereEqualTo(FIREBASE_FIRESTORE_PRODUCTS_CATEGORY_FIELD, FIREBASE_FIRESTORE_BENEFICIAL_PRODUCTS_CATEGORY_FIELD_VALUE)
